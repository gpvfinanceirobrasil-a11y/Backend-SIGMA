...
function handlePost(e) {
  try {
    ...
    if (action === 'uploadBase') return jsonResponse(uploadRows(CONFIG.SHEETS.BASE, body.rows || []));
    if (action === 'uploadInadimplencia') return jsonResponse(uploadRows(CONFIG.SHEETS.INADIMPLENCIA, body.rows || []));
    if (action === 'setup') return jsonResponse(setup());
